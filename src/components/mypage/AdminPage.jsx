...
export function AdminPage({ children }) {
    ...
    const getUserList = async () => {
        try {
            const getUserListResponse = await axios.get(`${process.env.REACT_APP_BACKEND_LINK}/mypage/admin?userid=${userid}`);
    ...
    const removeUser = async (userid) => {
        try {
            console.log(userid)
            const removeUserResponse = await axios.delete(`${process.env.REACT_APP_BACKEND_LINK}/mypage`, {
                data: { 
                    userid: userid
                }
            });
