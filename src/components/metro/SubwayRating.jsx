...
export function SubwayRating({query , token}) {
    ...
    const getRatingData = async() => {
      try {
        const getRatingDataResponse = await axios.get(`http://localhost:5000/search/rating?query=${query}`);
    ...
    const addRating = async() => {
      ...
      try {
        const addRatingResponse = await axios.post(`http://localhost:5000/search/rating`, {
          query : query,
          score : starScore,
          userid : userid,
        })
